 ...
 */
function ActionInteractionFramework()
{
    var actionData = {};
    ...
    function executeAction(actionName, rowContext, restApiResponse)
    {   

        var actionEntry = actionData[actionName];
        if(actionEntry[actionName].api)
        {
            if(actionEntry[actionName].api.userInput){
                var userInput = actionEntry[actionName].api.userInput; 
                var requestPayload = JSON.parse(JSON.stringify(actionEntry[actionName].api.rest.requestPayload));
            ...
            if(actionEntry[actionName].api.rest)
            { 
                restApiCall(actionEntry[actionName], processPayload(actionEntry[actionName].api.rest.requestPayload), restApiResponse);           
            }
        } else
            return null;
    };
    ...
    function executeActionWithUserInput(actionName, rowContext, userInput, restApiResponse)
    {
        ...
        if(actionEntry[actionName].api)
        {
            if(actionEntry[actionName].api.rest)
            { 
                restApiCall(actionEntry[actionName], processPayload(actionEntry[actionName].api.rest.requestPayload, userInput, rowContext), rowContext, restApiResponse);
            }
        } else
            return null;
    };
    ...
    function getHostName(){
        return "https://fuscdrmsmc349-fa-ext.us.oracle.com";
    }
