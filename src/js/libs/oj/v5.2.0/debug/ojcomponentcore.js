...
define(['ojs/ojcore', 'jquery', 'jqueryui-amd/widget', 'jqueryui-amd/unique-id', 'jqueryui-amd/keycode', 'jqueryui-amd/focusable', 'jqueryui-amd/tabbable', 'ojs/ojmessaging', 'ojs/ojcustomelement'], function(oj, $) {
    ...
    function _getComponentElementByNode(node, mtAccessCompositeInternals) {
        ...
        if (containingComposite) { // node is in or is a composite, return composite
            ...
            if (node.parentNode instanceof Element && node.parentNode.hasAttribute('data-oj-surrogate-id')) { // internal component is a popup
    ...
    (function() // BaseComponent wrapper function, to keep "private static members" private
        {
            ...
            $.widget('oj.' + _BASE_COMPONENT, {
                ...
                _triggerCustomEvent: function(type, event, data) {
                    var eventName, detail = {},
                        bubbles, cancelable, rootElement = this._getRootElement();
    ...
    oj.ComponentMessaging.prototype._getResolvedMessagingDisplayOptions = function() {
            var artifactsByDisplayType = {},
                ...
                compPH = this._component.options['placeholder'],
                artifact, keys, key, i, j,
    ...
    oj.MessagingStrategy.prototype.GetConverterHint = function() {
        var hints = [],
            mc = this._getMessagingContent(),
            converterHint = mc && mc.converterHint;
        if (converterHint) {
    ...
    oj.DomUtils._cleanElementHtml = function(node) {
        var children = node.childNodes,
            child, attrs, attr, childHasAttr, i;
    ...
    oj.GestureUtils.stopDetectContextMenuGesture = function(rootNode) {
        $(rootNode).off(oj.GestureUtils._EVENT_NAMESPACE)
            .removeClass("oj-menu-context-menu-launcher")[0].removeEventListener("click", rootNode._clickListener, true);
